...
def main():
    ...
    for fold, (train_idx, dev_idx) in enumerate(gkf.split(X, y, groups), 1):
        clf = Pipeline([
            ...
            ('fs', SelectFromModel(LogisticRegression(penalty='l1', solver='liblinear', C=0.5, random_state=42))),
            ('rf', RandomForestClassifier(max_depth=4, min_samples_leaf=5, random_state=42, class_weight='balanced'))
        ])
        ...
        for i, idx in enumerate(dev_idx):
            ...
            pauses_dev.append({
                "turn_id": tid, "dur": dur, "label": label, "p": dev_probs[i]
            })
    ...
    non_zero = np.sum(clf.named_steps['fs'].estimator_.coef_ != 0)
